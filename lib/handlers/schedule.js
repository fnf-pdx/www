  ...
  function save(request, reply) {
    ...
    ddb.putItem(
      ...
      function(err) {
        if (err) {
          console.log(err);
          form(request, reply, {status: 'failed', message: 'waiting list entry failed'});
          return;
        }
        ...
        ddb.deleteItem(
          {
            TableName: 'waitingList',
            Key: {team: {S: request.payload.team1}}
          },
          function(err) {
            if (err) {
              console.log(err);
              form(request, reply, {status: 'failed', message: 'failed to remove ' + team1 + ' from waiting list'});
            }

            ddb.deleteItem(
              {
                TableName: 'waitingList',
                Key: {team: {S: request.payload.team2}}
              },
              function(err) {
                if (err) {
                  console.log(err);
                  form(request, reply, {status: 'failed', message: 'failed to remove ' + team2 + ' from waiting list'});
                }
